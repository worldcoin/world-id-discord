...
export const Captcha = memo(function Captcha(props: {className?: string; variant?: 'primary' | 'flat'}) {
  ...
  return (
    ...
    >
      <label
        className={cn('grid items-center grid-flow-col auto-cols-max gap-x-3 px-4 py-2 cursor-pointer select-none', {
      ...
      >
        ...
        <span
          className={cn('relative w-5 h-5 grid items-center justify-center outline-none border rounded-full', {
            'border-ffffff': variant === 'primary',
            'border-183c4a': variant === 'flat',
          })}
        >
          {checked && <Icon className="w-3 h-3" name="check" />}
        </span>

        <span>I’m a unique person</span>

        <Icon className="w-10 h-10" noMask name="world-id" />
      </label>
    </div>
  )
})
